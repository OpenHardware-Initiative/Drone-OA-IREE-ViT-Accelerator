...
class TRAINER:
    def __init__(self, args=None):
        ...
        if self.model_type == 'ITALSTM':
            self.model = model_library.ITALSTM().to(self.device).float()
        elif self.model_type == 'ITAConformer':
            self.model = model_library.ITAConformer().to(self.device).float()
