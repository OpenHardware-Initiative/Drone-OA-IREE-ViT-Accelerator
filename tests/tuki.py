...
class PyTorchDataExtractor:
    ...
    def _get_hook(self, name):
        def hook(module, input_tensors, output_tensor):
            # <-- FIX 1: Capture ALL input tensors, not just the first one.
            # This is crucial for operations like matmul that have two inputs.
    ...
    def extract_and_capture(self, dummy_input):
        ...
        for name, module in self.model.named_modules():
            ...
            # NOTE: Removed ReLU due to operator fusion 
            #elif isinstance(module, torch.nn.quantized.ReLU):
            #     self.captured_data[name] = {'type': 'ReLU'}
            #     module.register_forward_hook(self._get_hook(name))

            # <-- FIX 2: We ONLY hook the modules we can trust: Linear, ReLU, and your custom_softmax.
            # We will get matmul inputs/outputs indirectly.
            elif 'attention_blocks' in name and name.endswith('custom_softmax'):
                self.captured_data[name] = {'type': 'Softmax'}
                module.register_forward_hook(self._get_hook(name))
            
            # NOTE: Not reliable.
            # Hook the second matmul (AV) to get its output context vector
            #elif 'attention_blocks' in name and name.endswith('matmul2'):
            #    self.captured_data[name] = {'type': 'MatMul_AV'}
            #    module.register_forward_hook(self._get_hook(name))
...
def organize_params_by_block(flat_params):
    ...
    for name, params in flat_params.items():
        ...
        if match:
            ...
            if block_type == 'ffn':
                if layer_key == 'fc1':
                    organized[block_type][int(d['index'])]['fc1'] = params
                elif layer_key == 'fc2':
                    # The input to fc2 is the post-ReLU tensor.
                    post_relu_tensor_dict = params['input'][0]
                    
                    # <-- THE FIX: Create a complete 'relu' dictionary
                    # with both 'input' and 'output' keys.
                    organized[block_type][int(d['index'])]['relu'] = {
                        'type': 'ReLU',
                        'input': post_relu_tensor_dict,  # For scale calculation
                        'output': post_relu_tensor_dict # For verification
                    }
                    organized[block_type][int(d['index'])]['fc2'] = params
                continue
...
def translate_torch_scales_to_hw_params(attn_block, ffn_block, H):
    ...
    for key, layer in all_layers.items():
        if layer.get('type') == 'Linear':
            if key == 'fc1':
                # For the fused fc1+ReLU, the true output scale is the post-ReLU scale.
                s_output = ffn_block['relu']['output']['scale']
            else:
                # For all other linear layers, the output scale is on the layer itself.
                s_output = layer['output_scale']
            
            s_input = layer['input'][0]['scale']
            s_weight = layer['weight']['scale']
            
            effective_scale = (s_input * s_weight) / s_output
            # --- END OF FIX ---
    ...
    if 'relu' in ffn_block:
        relu_layer = ffn_block['relu']
        # In a fused LinearReLU, the input scale to the conceptual ReLU is the same as the
        # output scale of the Linear part. PyTorch makes the final output scale the same.
        input_scale_relu = relu_layer['input']['scale'] # From our placeholder
        output_scale_relu = relu_layer['output']['scale']
        
        effective_scale = input_scale_relu / output_scale_relu # Should be ~1.0
        
        mult, shift = calculate_hw_params(effective_scale)
        # Your PyITA simulator expects these keys for its `apply_activation` function
        hw_params['relu'] = {'mult': mult, 'shift': shift, 'add': relu_layer['output']['zp'], 'q_1': 0, 'q_b': 0, 'q_c': 0}
...
def main(args):
    ...
    for block_index in sorted(organized_params['attention'].keys()):
        ...
        output_dir = f"pytorch_test_vectors_block_{block_index}"
