...
import sys
import os
# Add the monorepo root directory to the Python path.
# This MUST be done before importing any other project modules.
...
from fpga_link import pack_frame, unpack_reply, PORT, FPGA_IP
from utils import AgileCommandMode, AgileQuadState
# from user_code_host import compute_command_state_based
...
class AgilePilotNode:
    ...
    def inference_thread(self, img, state):
        ...
        try:
            # --- START OF FIX ---
            # The model expects a 60x90 image. We must resize the incoming image.
            h, w = (60, 90)
            img_resized = cv2.resize(img, (w, h), interpolation=cv2.INTER_AREA)
            # --- END OF FIX ---

            # Now, process the RESIZED image, not the original 'img'
            img_processed = np.clip(img_resized / self.depth_im_threshold, 0, 1)
            ...
            # This call will now succeed because depth_u8 is 60x90.
