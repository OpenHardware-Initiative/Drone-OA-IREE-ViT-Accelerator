...
class ITASelfAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # Use the functional wrappers for the matmuls
        attn_logits = self.matmul_qk.mul_transpose(Q_r, K_r)
        
        # In QAT, this will be fake-quantized. After conversion, it's an integer operation.
        attn_weights_int = self.softmax(attn_logits.int_repr()) 
        
        # Re-quantize softmax output to be compatible with the next matmul
        attn_weights_q = torch.quantize_per_tensor(
            attn_weights_int.float(), attn_logits.q_scale(), attn_logits.q_zero_point(), torch.quint8
        )
