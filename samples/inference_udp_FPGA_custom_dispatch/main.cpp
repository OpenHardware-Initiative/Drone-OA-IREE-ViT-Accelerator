...
int main(int argc, char** argv) {
    ...
    // 1. Get the path to the running executable.
    std::filesystem::path exe_path(argv[0]);

    // 2. Get the directory containing the executable.
    std::filesystem::path exe_dir = exe_path.parent_path();

    // 3. The model's FILENAME is injected by CMake. Join them to get the full path.
    std::filesystem::path vmfb_path = exe_dir / MODEL_VMFB_FILENAME;

    if (!std::filesystem::exists(vmfb_path)) {
        std::cerr << "Error: Model file '" << MODEL_VMFB_FILENAME
                  << "' not found in the executable's directory: " << exe_dir
                  << std::endl;
        return 1;
    }
